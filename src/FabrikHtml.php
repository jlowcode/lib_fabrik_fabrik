...
//			self::stylesheet($path);
	 ...
	 * @param   string $file  JS File path relative to root without .js extension e.g. 'media/fabrik/com_fabrik/js/list'
	 *
	 * @since   3.0b
	 *
	 * @return  void
	...
	{
		return self::isDebug() ? 'media/fabrik/com_fabrik/js' : 'media/fabrik/com_fabrik/js/dist';
	}
	...
	public static function calendar()
	{
		...
		/* We have to reload the style to pass along the attributes */
		$wa->useStyle("com_fabrik.calendar", [], $attribs);
		$translation = static::calendartranslation();
		if ($translation)
		{
			$wa->addInlineScript($translation, ["position" => "after"], [], ["com_fabrik.calendar"]);
		}
...
*/
			$script = ["window.Fabrik = Object.assign({}, window.Fabrik);"];
			if (!self::inAjaxLoadedPage())
			{
				Text::script('COM_FABRIK_LOADING');

				$script[] = "\twindow.Fabrik.liveSite = '" . COM_FABRIK_LIVESITE . "';";
				$script[] = "\twindow.Fabrik.package = '" . $app->getUserState('com_fabrik.package', 'fabrik') . "';";
				$script[] = "\twindow.Fabrik.debug = " . (self::isDebug() ? 'true;' : 'false;');

				$script[] = self::tipInt();
			}

			$script[] = "\twindow.Fabrik.bootstrapped = true;";
			$script[] = "\twindow.Fabrik.jLayouts = Object.assign({}, window.Fabrik.jLayouts, " . json_encode(ArrayHelper::toObject(self::$jLayoutsJs)) . ");";		
			$src = implode("\n", $script);

			self::$framework = $src;

			FabrikHtml::addToElemInitScripts($src);
		}
	...
	{
		return 'media/fabrik/com_fabrik/js/' . $file;
	}
	...
	{
		$folder = 'media/fabrik/com_fabrik/js/lib/slick/';
		self::script('/lib/slick/slick.js');
		FabrikHtml::stylesheet(COM_FABRIK_LIVESITE . 'media/fabrik/com_fabrik/js/lib/slick/slick.css');
		FabrikHtml::stylesheet(COM_FABRIK_LIVESITE . 'media/fabrik/com_fabrik/js/lib/slick/slick-theme.css');

		$folder = 'media/fabrik/com_fabrik/js/lib/elevatezoom-plus/';
	...
	public static function autoComplete($htmlId, $elementId, $formId, $plugin = 'field', $opts = array())
	{
		...
		*/
		...
		$script            = [];
		$script[]          = "window.FabrikAdmin = window.FabrikAdmin || {}; FabrikAdmin = window.FabrikAdmin;";
		$script[]          = "'FabrikAdmin.model.fields.autocomplete'.split('.').reduce((acc, key)=>(acc[key] = acc[key] ?? {}), window);";
		$script[]          = "if (FabrikAdmin.model.fields.autocomplete['$htmlId'] === undefined) {";
		$script[]          = "\tFabrikAdmin.model.fields.autocomplete['$htmlId'] = new FbAutoComplete('$htmlId', $str);";
		$script[]          = "}";
		$script            = implode("\n", $script);

		Factory::getApplication()->getDocument()->getWebAssetManager()->useScript("com_fabrik.autocomplete");
		FabrikHtml::addToElemInitScripts($script, ['FbAutoComplete' => '@fbautocomplete']);
	}
	...
	public static function autoCompleteOptions($htmlId, $elementId, $formId, $plugin = 'field', $opts = array())
	{
		...
		$json->url .= $app->isClient('administrator') ? '&task=plugin.pluginAjax' : '&view=plugin&task=plugin.pluginAjax';
	...
	public static function atWho($selector, $placeHolders = array())
	{
		...
		if (!array_key_exists($key, self::$atWho))
		{
			...
			FabrikHtml::stylesheet('media/fabrik/com_fabrik/js/lib/at/' . $css, 'atWho');
...
EOT;
		...
		if (!array_key_exists($type, self::$helperpaths))
		{
			...
			switch ($type)
			{
				case 'image':
					...
					self::$helperpaths[$type][] = COM_FABRIK_BASE . 'media/fabrik/com_fabrik/images/';
	...
	private static function getWebAssets() {
		...
		foreach ($existingAssets as $assetType => $assets) {
			foreach ($assets as $asset) {
				...
				if ($assetType == 'script' && ($asset->getOption("module", false) === true)) {
					$webAsset['attributes']['type'] = 'module';
				}
	...
	public static function LoadAjaxAssets() {
		...
		(async () => {
		    try {
		        const { insert_scripts_and_styles } = await import("@fbajaxassets");
		        insert_scripts_and_styles(' . json_encode($ajaxAssets, JSON_UNESCAPED_SLASHES) . ');
		    } catch (error) {
		        console.error("Failed to import @fbajaxassets:", error);
		    }
		})();
		</script>';
	}
	...
	public static function saveWebAssets() {
		...
		/* We are going to save the language strings as well */
		$session->set("fabrik.scriptStrings", Text::getScriptStrings());
	}
	/**
	 ...
	 * @return  void
	 */
	public static function addToElemInitScripts($script, $imports = [])
	{
