...
//	Does not get called when needed, so keep it or remove ?
	...
	public static function canPdf($puke = true) {
		...
		$pdfLib = $config->get('fabrik_pdf_lib', 'dompdf');

		if ($pdfLib == "dompdf") {  
			$file = COM_FABRIK_LIBRARY . '/vendor/vendor/dompdf/dompdf/composer.json';
			$found = File::exists($file);
			if ($found and class_exists("Dompdf") == false) {
				/* Run the autoloader */
				include_once(COM_FABRIK_LIBRARY . '/vendor/vendor/autoload.php');
			}
		} else {
			$file = COM_FABRIK_LIBRARY . '/vendor/vendor/mpdf/mpdf/composer.json';
			$found = File::exists($file);
			if ($found and class_exists("Mpdf") == false) {
				/* Run the autoloader */
				include_once(COM_FABRIK_LIBRARY . '/vendor/vendor/autoload.php');
			}
		}
